...
from gitingest.utils.auth_utils import resolve_token
...
async def ingest_async(
    ...
    *,
    ...
    # enrich query with ignore patterns & branch selection
    _apply_gitignores(query, apply_gitignores=not include_gitignored)
...
def _apply_gitignores(query: IngestionQuery, *, apply_gitignores: bool) -> None:
    """Update `query.ignore_patterns` in-place if *apply_gitignores* is True.

    Parameters
    ----------
    query : IngestionQuery
        The query to update.
    apply_gitignores : bool
        If True, apply the gitignores to the query.

    """
    if not apply_gitignores:
        return
